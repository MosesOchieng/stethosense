    ...
    <main className="min-h-[100dvh] overflow-hidden bg-[#f6f4ef] text-[#112a35]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <header className="sticky top-0 z-40 border-b border-[#dce2dd] bg-[#f6f4ef]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[70px] max-w-[1240px] items-center justify-between px-5 md:px-8">
          <button type="button" onClick={() => go("top")} className="flex items-center gap-2 text-left" aria-label="StethoSense home" data-testid="button-home">
          ...
          <nav className="hidden items-center gap-7 text-[12px] font-medium text-[#50656a] md:flex">
            {navItems.map(([label, id]) => <button type="button" key={id} onClick={() => go(id)} className="transition-colors hover:text-[#0e7370]" data-testid={`nav-${id}`}>{label}</button>)}
          </nav>
          <button type="button" onClick={() => setDemoOpen(true)} className="hidden rounded-full bg-[#102f3a] px-5 py-2.5 text-[12px] font-bold text-[#f7f5ef] transition-transform hover:-translate-y-0.5 md:block" data-testid="button-demo-header">Request a demo <ArrowRight className="ml-2 inline" size={13} /></button>
          <button type="button" onClick={() => setMenuOpen(!menuOpen)} className="rounded-full p-2 md:hidden" aria-label="Toggle menu" data-testid="button-menu">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button>
        </div>
         {menuOpen && <div className="border-t border-[#dce2dd] bg-[#f6f4ef] px-5 py-4 md:hidden">{navItems.map(([label, id]) => <button type="button" key={id} onClick={() => go(id)} className="block w-full py-3 text-left text-sm" data-testid={`mobile-nav-${id}`}>{label}</button>)}<button type="button" onClick={() => setDemoOpen(true)} className="mt-2 w-full rounded-full bg-[#102f3a] py-3 text-sm font-bold text-white" data-testid="button-demo-mobile">Request a demo</button></div>}
      </header>
      ...
      <section id="top" className="ss-noise relative mx-auto grid max-w-[1240px] items-center gap-12 px-5 pb-20 pt-16 md:grid-cols-[.9fr_1.1fr] md:px-8 md:pb-28 md:pt-24">
        ...
        <div className="relative min-h-[430px] md:min-h-[580px]">
          <div className="absolute inset-5 rounded-[42%_42%_12%_12%] bg-[#dce9e3]" />
           <img src="/images/stethosense-hero.jpg" alt="StethoSense connected stethoscope" className="relative h-[430px] w-full rounded-[32px] object-cover mix-blend-multiply md:h-[580px]" data-testid="image-hero" />
      ...
      <footer className="bg-[#0b242d] px-5 py-14 text-[#edf4ee] md:px-8"><div className="mx-auto flex max-w-[1240px] flex-col justify-between gap-10 md:flex-row md:items-end"><div><div className="flex items-center gap-2 text-sm font-bold"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#0e7370]"><HeartPulse size={16} /></span> StethoSense</div><p className="mt-5 max-w-xs text-sm leading-6 text-[#abc1bd]">We’re building the technology to help every heartbeat be heard.</p></div><div className="flex flex-wrap gap-3"><button type="button" onClick={() => setDemoOpen(true)} className="rounded-full bg-[#8dd8ca] px-5 py-3 text-xs font-bold text-[#102f3a]" data-testid="button-partner">Partner with us</button><button type="button" onClick={() => setDemoOpen(true)} className="rounded-full border border-[#527177] px-5 py-3 text-xs font-bold" data-testid="button-contact">Contact StethoSense</button></div></div><div className="mx-auto mt-12 max-w-[1240px] border-t border-[#294b53] pt-5 text-[10px] text-[#78928e]">© 2026 StethoSense Technologies · Designed for the people who listen first.</div></footer>

      {demoOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-[#102f3a]/70 px-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="demo-title"><div className="relative w-full max-w-md rounded-3xl bg-[#f6f4ef] p-7 text-[#102f3a] shadow-2xl"><button type="button" onClick={() => { setDemoOpen(false); setSubmitted(false); }} className="absolute right-5 top-5 rounded-full p-2 hover:bg-[#e2e9e3]" aria-label="Close demo request" data-testid="button-close-demo"><X size={18} /></button>{submitted ? <div className="py-10 text-center" role="status"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-[#dceee7] text-[#0e7370]"><Check /></div><h2 id="demo-title" className="mt-5 font-['Playfair_Display'] text-3xl">We’ll be in touch.</h2><p className="mt-3 text-sm text-[#637679]">Thanks for your interest in bringing clearer listening to your team.</p><button type="button" onClick={() => { setDemoOpen(false); setSubmitted(false); }} className="mt-7 rounded-full border border-[#aebfba] px-5 py-2.5 text-xs font-bold hover:bg-white" data-testid="button-close-success">Close</button></div> : <><p className="text-[11px] font-bold uppercase tracking-[.2em] text-[#0e7370]">Start a conversation</p><h2 id="demo-title" className="mt-3 font-['Playfair_Display'] text-4xl tracking-[-.04em]">See StethoSense in practice.</h2><p className="mt-3 text-sm leading-6 text-[#637679]">Tell us where you work and we’ll arrange a focused product walkthrough.</p><form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="mt-7 space-y-3"><label className="sr-only" htmlFor="demo-name">Your name</label><input id="demo-name" name="name" required aria-label="Name" placeholder="Your name" autoComplete="name" className="w-full rounded-xl border border-[#cbd7d0] bg-[#fffefa] px-4 py-3 text-sm outline-none focus:border-[#0e7370]" data-testid="input-demo-name" /><label className="sr-only" htmlFor="demo-email">Work email</label><input id="demo-email" name="email" required type="email" aria-label="Email" placeholder="Work email" autoComplete="email" className="w-full rounded-xl border border-[#cbd7d0] bg-[#fffefa] px-4 py-3 text-sm outline-none focus:border-[#0e7370]" data-testid="input-demo-email" /><button type="submit" className="w-full rounded-xl bg-[#0e7370] py-3.5 text-sm font-bold text-white" data-testid="button-submit-demo">Request a demo <ArrowRight className="ml-2 inline" size={15} /></button></form></>}</div></div>}
    </main>
  );
}